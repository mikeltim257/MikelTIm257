...
class PersonInfo:
    ...
    def new_salary(self):
        """Возвращает зарплату, вычисляемую по формуле:
         1337*Возраст*суммарное кол-во вхождений трех
         наиболее часто встречающихся букв из списка подразделений"""
        new_list = []
        our_str = ''.join(self.subdivision)
        ...
        new_list.sort()
        salary = 1337 * self.age * sum(new_list[::-1][:3])
